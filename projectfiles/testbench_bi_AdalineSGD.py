...
from binclassifier import AdalineSGD
...
if __name__ == "__main__":
    ...
    y = np.where(y == 'Iris-versicolor', -1, 1)  # labeling 'versicolor' as 1 rest of them are -1
    ...
    # fitting model
    model = AdalineSGD(eta=0.01, n_iter=15, random_state=1)
    model.fit(X_std, y)
    ...
    ax1.set_title('1. AdalineSGD test: Costs per Epoch')
    ax1.set_xlabel('epochs')
    ax1.set_ylabel('costs')

    plot_decision_regions(X_std, y, model, resolution=0.02, ax=ax2)
    ax2.set_title('2. AdalineSGD test: Decision Region')
